...
#define MLX5_SET(typ, p, fld, v) \
	do { \
		...
		*((rte_be32_t *)(p) + __mlx5_dw_off(typ, fld)) = \
		rte_cpu_to_be_32((rte_be_to_cpu_32(/*将此成员的值转为cpu序*/*((u32 *)(p) + \
...
struct mlx5_ifc_general_obj_in_cmd_hdr_bits {
	u8 opcode[0x10];
	u8 reserved_at_10[0x20];/*预留两个0x10*/
	u8 obj_type[0x10];
	u8 obj_id[0x20];
	union {
		struct {
